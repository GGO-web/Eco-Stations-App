...
export function Header() {
  ...
  return (
    <AppBar
      elevation={0}
      style={{
        position: 'sticky', background: '#8EE4AF', color: '#05386B', height: '70px',
      }}
    >
      <Toolbar style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>

        <NavLink className="h-full " to="/">
          <img src="/ecofinder.svg" alt="Logo" className="h-full object-cover object-center  w-[300px] bg-white" />
        </NavLink>
        <div>
          <div className="flex gap-4 items-center">
            <Typography variant="h6" component="div" className="w-full hover:bg-dark hover:text-light-green transition rounded-xl">
              <NavLink className="text-xl font-bold py-2 px-3" to="/detailed">Detailed</NavLink>
            </Typography>

            {role === 'Service' && (
              <Typography variant="h6" component="div" className="w-full hover:bg-dark hover:text-light-green transition rounded-xl">
                <NavLink className="text-xl font-bold py-2 px-3" to="/services">Services</NavLink>
              </Typography>
            )}
